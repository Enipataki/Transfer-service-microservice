...
import { AccountServiceMock } from "../mock-data/account-service-mock";
import { TransactionServiceMock } from "../mock-data/transaction-service-mock";
import { NotificationServiceMock } from "../mock-data/notification-service-mock";
import { PaymentRailServiceMock } from '../mock-data/payment-rail-service-mock';
import { LimitServiceMock } from '../mock-data/limit-service-mock';
/**
...
export class TransferService {
     ...
     * Database operations
     */
    async createTransferRecord(input, fee, totalAmount, client, bulkTransferId) {
    ...
    async createBulkTransferRecord(id, reference, senderAccountId, totalAmount, totalFee, transferCount, transfers, client) {
        const query = `
    ...
    async createRecurringTransferRecord(input, nextExecution, client) {
    ...
    async failTransfer(transfer, reason) {
    ...
    // Compliance check
    async performComplianceChecks(input, fundSource) {
        //Implementation KYC/AML checks here
        // This would integrate with external compliance services
        const complianceResult = await this.limitService.checkAMLCompliance({ userId: input.senderAccountId, amount: input.amount, recipientAccountId: input.recipientAccountId, transactionType: input.type, sourceOfFunds: fundSource });
        if (!complianceResult.approved) {
            throw new Error(`Compliance check failed: ${complianceResult}`);
        }
    }
    async recordExternalTransactionReference(transferId, externalReference, processedAt) {
        const query = `
        UPDATE transfers
    ...
    async validateTransfer(input, transactionType, fundSource) {
        ...
        await this.performComplianceChecks(input, fundSource);
    }
    //Idempotency management
    async checkIdempotency(key) {
        return await redis.get(`idempotency:${key}`);
    }
    async storeIdempotency(key, data) {
    ...
    async processIntraBankTransfer(transfer) {
        try {
    ...
    async processInterbankTransfer(transfer, externalReference) {
        try {
    ...
    async processTranfer(transferId, externalReference) {
        try {
     ...
     */
    async createTransfer(input, fundSource, idempotencyKey) {
        ...
        return await db.transaction(async (client) => {
            ...
            await this.validateTransfer(input, 'bulk', fundSource);
            // 2. Calculate fees
     ...
     */
    async createdBulkTransfer(input, fundSource, idempotencyKey) {
        if (idempotencyKey) {
        ...
        return await db.transaction(async (client) => {
            ...
            for (const transferInput of input.transfers) {
                ...
                await this.validateTransfer(fullInput, 'single', fundSource);
     ...
     */
    async createRecurringTransfer(input, fundSource) {
        return await db.transaction(async (client) => {
            //validate the recurring transfer
            await this.validateTransfer({ ...input, type: input.recipientBankCode ? 'INTERBANK' : 'INTRA_BANK' }, 'single', fundSource);
