export class TransactionServiceMock {
  ...
  async updateTransactionStatus(transactionId: string, status: 'pending' | 'completed' | 'failed' | 'reversed') {
  ...
  async getTransactionHistory(accountId: string, limit = 50, offset = 0) {
    ...
    const accountTransactions = allTransactions.filter(
      tx => tx.senderAccountId === accountId || tx.recipientAccountId === accountId
    );
