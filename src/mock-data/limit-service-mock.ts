export class LimitServiceMock {
  ...
  async validateTransferLimits(validationData: {
    ...
    amount: number;
    currency: string;
    transactionType: 'single' | 'bulk' | 'recurring';
    recipientType?: "INTRA_BANK" | "INTERBANK";
  }) {
  ...
  async resetUserLimits(userId: string, resetType: 'daily' | 'weekly' | 'monthly' | 'all') {
    const userLimit = this.userLimits.get(userId);
    if (!userLimit) {
      throw new Error('USER_LIMITS_NOT_FOUND');
    }
  ...
  private getNextLimitReset(): { daily: Date; weekly: Date; monthly: Date } {
    const now = new Date();
  ...
  async getLimitUsageReport(userId: string, period: 'day' | 'week' | 'month') {
